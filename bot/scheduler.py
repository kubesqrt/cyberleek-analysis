...
from . import config as C, data as D, universe as UV, signals as SG, thesis as TH, monitors as MO, safety as SF, report as RP

class State:
    universe = None; panel = None; signals = []; waves = []; asof = None; plans = {}; theses = {}
...
def refresh(state, limit=None, log=print):
    ...
    MO.save_scan(state.asof, {"signals": state.signals, "waves": state.waves}); log(f"scan {state.asof}: {len(state.signals)} signals, {len(state.waves)} waves")
    return state
...
def daily_messages(state):
    msgs = [RP.scan_summary(state.asof, state.signals, state.waves)]
